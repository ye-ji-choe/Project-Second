...
public class Cam : MonoBehaviour
{
    ...
    private Transform focusingTaget = null;
    ...
    private void FixedUpdate()
    {
        ...
        Vector3 position = Vector3.Lerp(transform.position, targetPosition, Time.fixedDeltaTime * lerpSpeed);
    ...
    public void OnFocus()
    {
        ...
        {
            focusingTaget = hit.transform;
            isFocusing = true;
            //포커싱하는 순간 포커싱된 물체를 정면으로 쳐다보는 코드
            Vector3 direction = focusingTaget.position - transform.position;
            ...
            targetPosition = focusingTaget.position - (targetRotation * Vector3.forward * focusDistance);
        }
    }
    ...
    {
        if (isFocusing && focusingTaget != null)
        {
            //1. 휠로 스크롤하면 줌 거리 조절
